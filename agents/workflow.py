...
class AgentWorkflow:
    ...
    def _check_relevance_step(self, state: AgentState) -> Dict:
        ...
        if classification == "CAN_ANSWER":
            ...
            return {
                "is_relevant": False,
                "draft_answer": f"This question ({state['question']}) isn't related (or there's no data) for your query. Please ask another question relevant to the uploaded document(s).",
            }

    def _decide_after_relevance_check(self, state: AgentState) -> str:
        decision = "relevant" if state["is_relevant"] else "irrelevant"
        logger.info(f"_decide_after_relevance_check -> {decision}")
        return decision
    ...
    def _research_step(self, state: AgentState) -> Dict:
        logger.info(f"Entered _research_step with question='{state['question']}'")
        result = self.researcher.generate(state["question"], state["documents"])
        logger.info("Researcher returned draft answer.")
        return {"draft_answer": result["draft_answer"]}

    def _verification_step(self, state: AgentState) -> Dict:
        logger.info("Entered _verification_step. Verifying the draft answer...")
        result = self.verifier.check(state["draft_answer"], state["documents"])
        logger.info("VerificationAgent returned a verification report.")
        return {"verification_report": result["verification_report"]}

    def _decide_next_step(self, state: AgentState) -> str:
        verification_report = state["verification_report"]
        logger.info(
            f"_decide_next_step with verification_report='{verification_report}'"
        )
        if (
            verification_report.supported == "NO"
            or verification_report.relevant == "NO"
        ):
            logger.info("Verification indicates re-research needed.")
            return "re_research"
        else:
